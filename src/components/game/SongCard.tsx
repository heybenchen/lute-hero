...
export function SongCard({ song }: SongCardProps) {
  return (
    <div className="card w-full max-w-[280px] mx-auto">
      ...
      {song.effect ? (
        <div className="mb-2 p-1.5 rounded text-xs flex items-center gap-1.5"
      ...
      <div className="flex justify-center gap-3">
        {song.slots.map((slot, idx) => (
          <div
            key={idx}
            className="w-20 h-20 aspect-square shrink-0 rounded-lg flex flex-col items-center justify-center text-xs"
            style={{
          ...
          >
            {slot.dice ? (
              <div className="text-center">
                ...
                <div className="font-bold text-[10px] text-parchment-300">{slot.dice.genre}</div>
              </div>
            ) : (
              <div className="text-parchment-500/40 text-sm">Empty</div>
            )}
          </div>
        ))}
      </div>
